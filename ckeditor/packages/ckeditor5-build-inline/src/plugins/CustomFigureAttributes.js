/**
 * 参考: https://ckeditor.com/docs/ckeditor5/latest/framework/guides/deep-dive/conversion/conversion-preserving-custom-content.html <br/>
...
export class CustomFigureAttributes {
  ...
  afterInit() {
	...
	setupCustomAttributeConversion( 'img', 'imageInline', 'data-mathml', editor );
	setupCustomAttributeConversion( 'img', 'imageInline', 'class', editor );

	setupCustomAttributeConversion( 'img', 'imageBlock', 'data-mathml', editor );
	setupCustomAttributeConversion( 'img', 'imageBlock', 'class', editor );
...
function downcastCustomClassesToFigure( modelElementName ) {
  return dispatcher => dispatcher.on( `insert:${ modelElementName }`, ( evt, data, conversionApi ) => {
	...
	let attribute = modelElement.getAttribute( 'customClass' )
	if(attribute === undefined || attribute == null){
	  return
	}
	conversionApi.writer.addClass( attribute, viewFigure );
  } );
}
...
function downcastCustomClassesToChild( viewElementName, modelElementName ) {
  return dispatcher => dispatcher.on( `insert:${ modelElementName }`, ( evt, data, conversionApi ) => {
	...
	conversionApi.writer.addClass( attribute, viewElement );
  } );
}
...
function downcastAttribute( modelElementName, viewElementName, viewAttribute, modelAttribute ) {
  return dispatcher => dispatcher.on( `insert:${ modelElementName }`, ( evt, data, conversionApi ) => {
	...
	let attribute = modelElement.getAttribute( modelAttribute )
	if(attribute === undefined || attribute == null){
	  return
	}
	conversionApi.writer.setAttribute( viewAttribute, attribute, viewElement );
  } );
}
